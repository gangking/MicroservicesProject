...
<template>
  <el-form autocomplete="off" ref="loginForm" :rules="logRules" :model="loginForm" class="login-form" status-icon label-width="0">
    <el-form-item prop="username">
      <el-input v-model="loginForm.username" :placeholder="$t('login.username')" tabindex="1" autocomplete="off" prefix-icon="ibps-icon-user" @keyup.enter.native="handleLogin" />
    </el-form-item>

    <el-tooltip v-model="capsTooltip" :content="$t('login.capsTooltip')" placement="right" manual>
      <el-form-item prop="password">
        <el-input v-model="loginForm.password" autocomplete='new-password' :type="passwordType" :placeholder="$t('login.password')" tabindex="2" prefix-icon="ibps-icon-lock" @keyup.native="checkCapslock" @blur="capsTooltip = false" @keyup.enter.native="handleLogin">
...
export default {
      ...
      handler: function(val, oldVal) {
        if (val) {
          this.loginRules.username = [
            { required: true, trigger: 'blur', validator: this.validateMobile }
          ];
        } else {
          this.loginRules.username = [
            {
  ...
  methods: {
    ...
    loadValidCode(isBackfill) {
      this.$store
        .dispatch('ibps/account/getCaptcha')
        .then(data => {
          // 返回状态501，说明系统没有开启验证码
          if (data.state === 501) {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        //this.loginForm.password = encryptDES(DEFAULT_KEY, DEFAULT_KEY, this.loginForm.password);
        if (valid) {
          this.loading = true;
          ...
          this.$store
            .dispatch('ibps/account/loginByUsername', {
              vm: this,
              form: {
                ...this.loginForm,
                password: encryptDES(DEFAULT_KEY, DEFAULT_KEY, this.loginForm.password)
              }
            })
            .then(() => {
          ...
          // this.$message({
          //   message: this.$t('common.dialog.saveError'),  //登陆去掉重复的提示
          //   type: "warning"
          // });
          return false;
        }
