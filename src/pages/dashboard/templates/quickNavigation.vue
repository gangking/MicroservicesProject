      ...
      <div :style="{height:showHeight,width:'100%'}" class="nav-content">
        <ul>
          <li v-for="(d,i) in data" :key="i">
            <a :href="d.url" :target="d.target">{{ d.label }}</a>
          </li>
          <el-button icon="ibps-icon-add" type="primary" size="mini" @click="getFormData">添加</el-button>
        </ul>
      </div>
    </div>
    <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" :close-on-press-escape="false" title="编辑" append-to-body>
