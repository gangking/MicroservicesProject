<template>
  <div class="bg" id="monitoring-difaccount" v-loading="bgLoading">
    ...
    <div class="table-content" :style="{height:height+'px'}">
      ...
      >
        ...
        <el-table-column prop="percentage10" label="10-30%"></el-table-column>
        <el-table-column prop="percentage30" label="30-50%"></el-table-column>
        <el-table-column prop="percentage50" label="50-100%"></el-table-column>
        <el-table-column prop="percentage100" label="100%以上"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    routeTo(row) {
      ...
      this.monitorDetailObj["componet"] = "difaccount";
    },
    //日期装换
    ...
    transformData(data) {
      ...
      data.forEach(item => {
        ...
        for (const key in item) {
          if (key != "difaccountCountVo" && key != "difaccountPriceVo") {
            countObj[key] = item[key];
            priceObj[key] = item[key];
          }
        }
        for (const objKey in item) {
          if (objKey == "difaccountPriceVo") {
            let priceSum = item["difaccountPriceVo"];
            for (const price in objKey) {
              priceObj["percentage10"] = priceSum["price1"]; //10
              priceObj["percentage30"] = priceSum["price2"]; //30
              priceObj["percentage50"] = priceSum["price3"]; //50
              priceObj["percentage100"] = priceSum["price4"]; //100

              priceObj["name"] = " 差异金额(万元)";
            }
            changeData.push(priceObj);
          } else if (objKey == "difaccountCountVo") {
            let countSum = item["difaccountCountVo"];
            for (const count in objKey) {
              countObj["percentage10"] = countSum["count1"];
              countObj["percentage30"] = countSum["count2"];
              countObj["percentage50"] = countSum["count3"];
              countObj["percentage100"] = countSum["count4"];
    ...
    getTable(limit = 20, pageno = 1) {
      ...
      getDifaccountReport(params).then(response => {
        this.loading = false;
