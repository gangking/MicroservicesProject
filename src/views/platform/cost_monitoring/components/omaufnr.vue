<template>
  <div class="bg" id="monitoring-omaufnr" v-loading="bgLoading">
    ...
    <div class="table-content" :style="{height:height+'px'}">
      ...
      >
        ...
        <el-table-column prop="totalCount" label="跨期开打总数"></el-table-column>
        <el-table-column prop="zblje" label="补料金额(万元)"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    routeTo(row) {
      ...
      this.monitorDetailObj["componet"] = "omaufnr";
    },
    //日期装换
    ...
    getTable(limit = 20, pageno = 1) {
      ...
      getOmaufnrReport(params).then(response => {
        this.loading = false;
    ...
    //计算中间table高度
    getTableHeight() {
      if (document.querySelector("#monitoring-omaufnr")) {
        let height = document.querySelector("#monitoring-omaufnr").clientHeight;
        // console.log(height, "height整体");
        let btnHeight = document.querySelector(
          "#monitoring-omaufnr .header-btn"
        ).clientHeight;
        // console.log(btnHeight, "btnHeight整体");
        let inputHeight = document.querySelector(
          "#monitoring-omaufnr .header-input"
        ).clientHeight;
        //console.log(inputHeight, "inputHeight中间的输入框");
        let footerHeight = document.querySelector("#monitoring-omaufnr .footer")
          .clientHeight;
