<template>
  <div class="bg" id="monitoring-wipaufnr" v-loading="bgLoading">
    ...
    <div class="table-content" :style="{height:height+'px'}">
      ...
      >
        ...
        <el-table-column prop="days1" label="1-30天"></el-table-column>
        <el-table-column prop="days30" label="30-90天"></el-table-column>
        <el-table-column prop="days90" label="90-180天"></el-table-column>
        <el-table-column prop="days180" label="180-360天"></el-table-column>
        <el-table-column prop="days360" label="360天以上"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    routeTo(row) {
      ...
      this.monitorDetailObj["componet"] = "wipaufnr";
    },
    //日期装换
    ...
    transformData(data) {
      ...
      data.forEach(item => {
        ...
        for (const key in item) {
          if (key != "screen2PriceVo" && key != "screen2CountVo") {
            countObj[key] = item[key];
            priceObj[key] = item[key];
          }
        }
        for (const objKey in item) {
          if (objKey == "screen2CountVo") {
            let countSum = item["screen2CountVo"];
            for (const count in objKey) {
              countObj["days1"] = countSum["count1"];
              countObj["days30"] = countSum["count2"];
              countObj["days90"] = countSum["count3"];
              countObj["days180"] = countSum["count4"];
              countObj["days360"] = countSum["count5"];
              countObj["name"] = " 数量";
            }
            changeData.push(countObj);
          } else if (objKey == "screen2PriceVo") {
            let priceSum = item["screen2PriceVo"];
            for (const price in objKey) {
              priceObj["days1"] = priceSum["price1"]; //1-30
              priceObj["days30"] = priceSum["price2"]; //30-90
              priceObj["days90"] = priceSum["price3"]; //90-180
              priceObj["days180"] = priceSum["price4"]; //180-360
              priceObj["days360"] = priceSum["price5"]; //360
              priceObj["name"] = "金额(万元)";
            }
    ...
    getTable(limit = 20, pageno = 1) {
      ...
      getWipaufnrReport(params).then(response => {
        this.loading = false;
