  ...
  <div>
    ...
    <div class="header-input" ref="headerInput">
      ...
      >
        ...
        <el-form-item label="共享审核组" class="form-item" prop="gxshz">
          <el-select
            v-model="ruleForm.gxshz"
            filterable
            clearable
            multiple
            @change="changeHeight"
            placeholder="请选择共享审核组"
          >
            <el-option
              v-for="item in gxshzOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
        ...
        <el-form-item label="共享审核人" class="form-item" prop="gxshr">
          <div>
            <el-input
              @click.stop.native="personSelect"
              v-model="ruleForm.person"
              clearable
              @clear="clearPerson"
              placeholder="请选择共享审核人"
            ></el-input>
          </div>
        </el-form-item>
      </el-form>
    ...
    <div class="table-content" :style="{height:height+'px'}">
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="bs_CODE_" label="业务场景"></el-table-column>
        <el-table-column prop="orgtext_" label="事业部"></el-table-column>
        <el-table-column prop="bukrs_" label="公司代码"></el-table-column>

        <el-table-column prop="gxshz" label="共享审核组"></el-table-column>
        <el-table-column prop="gxshr" label="共享审核人"></el-table-column>
        ...
        <el-table-column prop="qjycldjl" label="期间累计已处理单据量"></el-table-column>
        <el-table-column prop="qjxzdjl" label="期间新增单据量"></el-table-column>
        <el-table-column prop="qjcscll" label="期间超时处理量"></el-table-column>
        <el-table-column prop="drxzdjl" label="当日新增单据量"></el-table-column>
        <el-table-column prop="drcldjl" label="当日处理单据量"></el-table-column>
        <el-table-column prop="dcldjlzc" label="待处理单据量-正常"></el-table-column>
        <el-table-column prop="dcldjlcs" label="待处理单据量-超时"></el-table-column>
        <el-table-column prop="dcldjl4H" label="待处理单据量-4h"></el-table-column>
        <el-table-column prop="dcldjl8H" label="待处理单据量-8h"></el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  data() {
    var validateGxshr = (rule, value, callback) => {
      if (value.length == 0 && this.ruleForm.gxshz.length == 0) {
        callback(new Error("共享审核组和共享审核人不能全部为空"));
      } else {
        callback();
      }
    };
    var validateGxshz = (rule, value, callback) => {
      if (value.length == 0 && this.ruleForm.gxshr.length == 0) {
        callback(new Error("共享审核组和共享审核人不能全部为空"));
      } else {
        callback();
      }
    ...
    return {
      ...
      rules: {
        ...
        gxshr: [{ validator: validateGxshr, trigger: "change" }],
        gxshz: [{ validator: validateGxshz, trigger: "change" }]
      },
      tableData: [], //数据
      currentPage: 1, //当前页
      pageSize: 20, // 每页多少条
  ...
  methods: {
    ...
    selectorData(data) {
      let copyArr = [];
      data.forEach(item => {
        this.ruleForm.gxshr.push(item.userno);
    ...
    loadData() {
      ...
      //共享审核组
      getShzList().then(response => {
        this.gxshzOptions = response.data.map(item => {
          return {
            value: item.id,
            label: item.name
          };
    ...
    getTable(limit = 20, pageno = 1) {
      ...
      this.computedHeight();
      shareWorkSearch(params).then(response => {
        this.loading = false;
