  ...
  <div class="boxTEST">
    ...
    <div class="diabox">
      <el-table border v-if="dialogTitle=='发票导入'" ref="multipleTable" @select="selectRow" @select-all="allSelectRow" @selection-change="handleSelectionChange" :data="tableData" tooltip-effect="dark" style="width: 100%" height="100%">
        ...
        <el-table-column align="center" prop="SALER_NAME" label="销方公司名称">
        </el-table-column>
...
export default {
  ...
  created() {
    this.dialogTitle = this.tagVal.openTitle;
    console.log('发票导入', this.tagVal, this.dialogTitle);
    this.onload();
  },
  data() {
    return {
      dialogTitle: this.tagVal.openTitle,
  ...
  methods: {
    ...
    importFunc() {
      this.activeInvoiceList.forEach((val, index, arr) => {
        ...
        // 循环导入
        this.multilineSaveFormData(info);
      });
    }
  }
};
