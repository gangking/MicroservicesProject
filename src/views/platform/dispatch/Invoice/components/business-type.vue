...
export default {
  ...
  created() {
    const { storageCache, businessEventId = null } = this;
    //   console.log('初始化business...');
    if (businessEventId) {
      // storageCache.deleteCache(businessEventId);
      const list = storageCache.getCache(businessEventId) || [];
    ...
    this.getBusinessTypeList({ businessEventId }).then(data => {
      ...
      this.$emit('action-event', 'setBusinessType', [data]);
    });
  },
  methods: {
    ...
    setLocalItem(item) {
      ...
      if (businessEventId) {
        ...
        storageCache.setCache(businessEventId, newArray);
    ...
    OnBusinessType(val) {
      ...
      this.$emit('action-event', 'selectBusinessType', [val]);
    },
