<template>
  <div class="codemirrorBox">
    <div class="codemirror">
      <div class="meng" v-if="!isEditable" title="禁止编辑"></div>
      <textarea ref="formula" class="cm-s-default" style="height:150px;" />
      </div>
...
export default {
  ...
  methods: {
    ...
    enterClick(info) {
      ...
      switch (this.isTxtNumConst) {
        ...
        case true:
          // 不能输入#$@%^&*<>￥危险字符字符
          var re1 = /^[^#$@%^&*<>￥]+$/;
          this.checkConstValue(info, re1, '不能输入#$@%^&*<>￥危险字符字符');
          break;
      }
    },
    ...
    logicInput(item,b) {
      ...
      this.setCodeBoxSetList(this.editor.getValue(), 'add', item);
    },
    // 输入逻辑符号
    clickFunctionNode(val, b) {
      if (val.key == 'contains' && b != '详情') {
        var list = [
          val,
          {
            id: '10',
        ...
        list.forEach(item => {
          if (item.type == 'btn') {
            this.logicInput(item,b);
          } else {
            this.insertFieldHand(item.val);
          }
        });
      } else {
        this.logicInput(val,b);
      }
    },
    // 手动输入处理方法
    insertFieldHand(name, b) {
      console.log('手动输入', name, b);
      var start = this.editor.getCursor();
