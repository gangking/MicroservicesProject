<template>
  <li class="tree-item-li">
    <div v-if="isEnd" :class="nodes.cengji==0?'line2 line2one':'line2'"></div>
    <div v-if="isEnd && isActive" class="line"></div>
    <i :class="isActive?'el-icon-minus':'el-icon-plus'" v-if="isEnd" @click="activeItem"></i>
    <span class="tree-item-label" @click="clickItem(nodes)">{{nodes.name}}</span>
    <ul v-if="isEnd" v-show="isActive" class="tree-item-ul">
      <my-tree v-for="(item, index) in nodes.nodes" :key="index" :nodes="item" @getclickchildren="clickchildren">
      </my-tree>
    </ul>
    <span class="status" v-if="!isEnd" @click="clickItem(nodes)">
      <span>
        <i :class="'icon-'+nodes.className2" :style="'color:'+nodes.color2+';'"></i>
      </span>
      <span>
        <i :class="'icon-'+nodes.className" :style="'background:'+nodes.color+';'"></i>
      </span>
    </span>
