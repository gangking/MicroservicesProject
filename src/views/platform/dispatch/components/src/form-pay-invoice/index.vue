<template>
  <el-dialog title="提示" :visible.sync="dialogVisible" width="24%" style="padding:10px;background: #9093994d;" append-to-body :modal="false" @close="handleClose">
    <div class="content">
      ...
      <div v-else>
        <p v-for="(item,key) in invoiceData" :key="key">
          公司代码：{{ item.bukrs }}
          <span class="con-left">应收款方：{{ item.lifnr }}</span>
        </p>
      </div>
    </div>
...
const { mapActions, mapState } = createNamespacedHelpers('ibps/compositeAssets');
