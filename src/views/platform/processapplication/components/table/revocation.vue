<template>
  <div class="tab">
    <div class="btn">
      ...
      <el-button size="mini" plain type="danger" class="elbtn" @click="deletedata">
        <i class="iconfont icon-uniE03D1 iconbtn" />
        <span class="icontext">删除表单</span>
      </el-button>
...
export default {
  ...
  methods: {
    ...
    detailvisible(row) {
      console.log("detailvisible22220000", row);
      const { appId, procInstId: instId, taskId, status } = row;
    ...
    getRow(row) {
      ...
      let copyInfo = {};
      row.forEach(item => {
        this.delIds.push(item.procInstId);
    ...
    deletedata() {
      return request({
        baseURL: BASE_GATEWAY_API,
        url: FORM_URL + "/bpm/v3/bpm/instance/remove?ids=" + this.delIds,
        method: "post",
        data: {
          // parameters: [
