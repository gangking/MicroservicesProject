  ...
  <el-row :gutter="10">
    <el-col v-if="!readonly" :span="8" border>
      <div class="grid-content">
        <tree ref="elTree" :height="height" :data="treeData" :multiple="false" :check-strictly="false" :check-on-click-node="true" node-key="id" @action-event="handleTreeAction" @getorgloadingflag="getorgloadingflag" />
      </div>
    </el-col>
...
import Tree from '../../components/tree/org-tree';
...
export default {
  ...
  methods: {
    getorgloadingflag(val) {
      this.$emit('getorgloadingflag', val);
    },
    init() {
      this.orgData = {};
    ...
    initTreeData() {
      ...
      newFindTreeData(
        ...
        .then(response => {
          ...
          this.treeData = Object.freeze(response.data);
        })
        .catch(() => {});
    },
    // 组织树事件
    ...
    handleBelongTo() {
      ...
      if (curNode === null || curNode.parent.key === undefined) {
        return;
      }
      this.orgData = {
