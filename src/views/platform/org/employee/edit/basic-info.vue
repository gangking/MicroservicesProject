...
import { genderOptions, statusOptions, isSuperOptions, userSecurityTextObj } from '../constants'
...
export default {
  ...
  data() {
    const validateAccount = (rule, value, callback) => {
      ...
      const re = /^[0-9a-zA-Z]{1,192}$/

      if (!re.test(value)) {
        callback(new Error('账号必须至少3位'))
      } else {
        callback()
      }
    }
    const validatePassword = (rule, value, callback) => {
      ...
      if (this.userSecurity.complexity.length === 0) {
        ...
        this.userSecurity.complexity.forEach(i => {
          if (complexityRegular[i].test(value)) {
    ...
    return {
      ...
      limit: 1, // 规定上传个数
      accept: 'image/jpeg,image/gif,image/png,image/jpg', // 规定上传类型
  ...
  methods: {
    handleGetDefaultUserSecurity() {
      getDefaultUserSecurity().then(response => {
        ...
        const complexityArr = response.data.complexity.split(',')
        const arr = []
        complexityArr.forEach(i => {
          arr.push(userSecurityTextObj[i])
        })
    ...
    validate(callback) {
      this.$refs[this.formName].validate((v) => { callback(v) })
    },
    /**
     * 重置字段
     */
