  ...
  <div class="bg" ref="main" v-loading="bgLoading" element-loading-text="发送中,请等待">
    ...
    <div class="header-input" ref="header_input">
      <el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="125px">
        ...
        <el-form-item label="接口编码">
          <el-input v-model="ruleForm.API_CODE" placeholder="请输入系统编码" clearable></el-input>
        </el-form-item>
        <el-form-item label="接口描述">
          <el-input v-model="ruleForm.API_DESC" placeholder="请输入系统描述" clearable></el-input>
        </el-form-item>
        <el-form-item label="所属系统">
          <el-input v-model="ruleForm.SYS_CODE " placeholder="请输入系统描述" clearable></el-input>
        </el-form-item>
        <el-form-item label="对方业务对象">
    ...
    <div class="table-content" :style="{height:height+'px'}">
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column prop="apiCode" label="接口编码"></el-table-column>
        <el-table-column prop="apiDesc" label="接口描述"></el-table-column>
        <el-table-column prop="sysCode" label="所属系统"></el-table-column>
        <el-table-column prop="boCode" label="对方业务对象"></el-table-column>
...
import {
  ...
  integInterfaceRemove
} from "@/api/platform/integrate/interface";
import integrateDialog from "@/views/platform/integrate/integrateInterface/components/dialog";
...
export default {
  ...
  methods: {
    ...
    handleBtn(command, index, row) {
      ...
      switch (command) {
        case "enabled":
          // let exchange = JSON.parse(JSON.stringify(row));
          // exchange["status"] = "1";
          integSysAddEdit({
            id: row.id,
            apiCode: row.apiCode,
            apiComments: row.apiComments,
            apiDesc: row.apiDesc,
            apiType: row.apiType,
            boCode: row.boCode,
            model: row.model,
        ...
        case "edit":
          ...
          break;
        case "stop":
          // let exchangeStop = JSON.parse(JSON.stringify(row));
    ...
    getTable(limit = 20, pageno = 1) {
      ...
      integInterfaceQuery(params).then(response => {
        this.loading = false;
