...
<script>
// checkCode TODO
import { manage, save, getJson, getJsonByTbl } from '@/api/platform/bo/boDef'
    ...
    return {
      ...
      treeContextmenus: [
        ...
        { icon: 'add', label: '添加下级业务对象', value: 'addSub', rights: (menu, data, isRoot) => {
          if (!this.nodeStatus(data, 'rights')) return false
          if (isRoot) true
  ...
  methods: {
    ...
    handleStepAction(boType, value) {
      ...
      if (boType === 'new') {
        ...
        if (boType === 'boDef') {
          if (this.checkSameNode(value, 'id')) {
    ...
    // 外部数据完成按钮
    complete(paramArr) {
      if (this.checkSameNode(JSON.parse(paramArr), 'code')) {
    ...
    // 添加数据
    changeData(value) {
      // if (this.boType === 'external' && this.treeData[0][this.childrenKey].length > 0) { return }
      ...
      this.tempTreeData = tempTreeData
