  ...
  <div class="tab">
    <div class="btn">
      <!-- <el-button size="mini" plain type="primary" class="elbtn" @click="agree">
        <i class="iconfont icon-uniE03F1 iconbtn"></i>同意
      </el-button>
      <el-button size="mini" plain type="danger" class="elbtn">
        <i class="iconfont icon-uniE01E iconbtn"></i>
        驳回
      </el-button>-->
      <div class="rightbtn">
        <el-dialog
...
export default {
  ...
  methods: {
    ...
    reLoadData() {
      this.$emit("getunchedata");
    },
    detailvisible(row) {
      console.log("detailvisible11111", row);
      this.checkNumber = row.procInstId;
      this.check()
    ...
    check() {
      return request({
        baseURL: BASE_GATEWAY_API,
        url: FORM_URL + "/bpm/v3/bpm/task/readCc?instId=" + this.checkNumber,
        method: "post",
        data: {}
      }).then(res => {
