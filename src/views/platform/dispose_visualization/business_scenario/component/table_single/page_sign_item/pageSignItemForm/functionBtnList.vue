...
export default {
  ...
  methods: {
    ...mapActions([
      'singleFormButtonItemDefinitionEdit',
      'singleFormButtonItemDefinitionAdd',
      'singleFormButtonItemDefinitionDelete',
      'singleFormButtonAll'
    ]),
    ...
    btnClick() {
      ...
      switch (this.btnType) {
        ...
        case '删除':
          if (JSON.stringify(this.activeBtn) != '{}' && this.activeBtn != null) {
            this.singleFormButtonItemDefinitionDelete({
      ...
      if (this.dialogTitle == '新增功能按钮') {
        if (this.activeAllBtn.length > 0) {
          for (var i = 0; i < this.activeAllBtn.length; i++) {
            ...
            item['slcfTabKey'] = this.thisId;
            item['slcfTabId'] = this.thisCode;
          }
          indexApi.singleFormButtonItemDefinitionAdd(this.activeAllBtn).then(res => {
            // this.onload();
            this.$emit('refreshBtn');
            this.activeBtn = {};
            this.handleClose();
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            this.singleFormButtonItemDefinitionEdit(this.ruleForm).then(res => {
              this.$emit('refreshBtn');
              this.activeBtn = {};
              this.handleClose();
            });
