  ...
  <tabBox :tabList="tabList" :activeName="activeName" @handleClick="handleClick">
    <div class="tableSlot" slot="tableSlot">
      <tableLayout :tbaleTitle="tbaleTitle" @btnClick="btnClick" :tableBtnList="tableBtnList" :tabelData="tabelData">
        <!-- 表单分配列表 -->
        <pageDefinitionList @refreshPage="multipleBusTypeFormSetDefinitionFunc" :thisId="id" :thisCode="code" :clickFlag="clickFlag" :buttonType="buttonType" v-if="activeName == 'pageDefinitionList'" slot="table" :tabelData="tabelData" />
        <!-- 功能按钮列表 -->
...
export default {
  ...
  methods: {
    ...mapActions([
      'multipleBusTypeBtnSetDefinition',
      'multipleBusTypeFormSetDefinition',
    ]),
    onload() {
      this.tableBtnList = require('./testData/tableBtnList.json').tableBtnList;
      this.multipleBusTypeBtnSetDefinition({
        bcKey: this.id
      }).then(res => {
        console.log('多行业务事务业务类型 功能按钮', res);
        this.btnTabel = res;
      });
    },
    multipleBusTypeFormSetDefinitionFunc(){
      this.tableBtnList = require('./testData/tableBtnList.json').tableBtnList;
      this.multipleBusTypeFormSetDefinition({
        bcKey: this.id
      }).then(res => {
        console.log('多行业务事务业务类型 表单分配', res);
        this.tabelData = res;
      });
    },
    handleClick(tab) {
      ...
      if (tab.label == '表单分配') {
        ...
        this.multipleBusTypeFormSetDefinitionFunc()
