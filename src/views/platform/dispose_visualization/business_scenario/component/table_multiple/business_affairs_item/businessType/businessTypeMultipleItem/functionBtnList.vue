...
export default {
  ...
  methods: {
    ...mapActions([
      'multipleBusTypeBtnSetDefinitionEdit',
      'multipleBusTypeBtnSetDefinitionAdd',
      'multipleBusTypeBtnSetDefinitionDelete',
      'singleFormButtonAll'
    ]),
    ...
    btnClick() {
      ...
      switch (this.btnType) {
        ...
        case '删除':
          if (JSON.stringify(this.activeBtn) != '{}' && this.activeBtn != null) {
            this.multipleBusTypeBtnSetDefinitionDelete({
      ...
      if (this.dialogTitle == '新增功能按钮') {
        if (this.activeAllBtn.length > 0) {
          for (var i = 0; i < this.activeAllBtn.length; i++) {
            ...
            item['bcKey'] = this.thisId;
            item['bcId'] = this.thisCode;
          }
          indexApi.multipleBusTypeBtnSetDefinitionAdd(this.activeAllBtn).then(res => {
            // this.onload();
            this.$emit('refreshBtn');
            this.activeBtn = {};
            this.handleClose();
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            this.multipleBusTypeBtnSetDefinitionEdit(this.ruleForm).then(res => {
              this.$emit('refreshBtn');
              this.activeBtn = {};
              this.handleClose();
            });
