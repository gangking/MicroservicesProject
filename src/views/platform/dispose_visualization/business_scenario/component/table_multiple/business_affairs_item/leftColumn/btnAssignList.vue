...
export default {
  ...
  methods: {
    ...mapActions([
      'multipleLeftButtonDefinitionEdit',
      'multipleLeftButtonDefinitionAdd',
      'multipleLeftButtonDefinitionDelete',
      'singleFormButtonAll'
    ]),
    ...
    btnClick() {
      ...
      switch (this.btnType) {
        ...
        case '删除':
          if (JSON.stringify(this.activeBtn) != '{}' && this.activeBtn != null) {
            this.multipleLeftButtonDefinitionDelete({
    ...
    submitForm(formName) {
      if (this.dialogTitle == '新增功能按钮') {
        if (this.activeAllBtn.length > 0) {
          ...
          indexApi.multipleLeftButtonDefinitionAdd(this.activeAllBtn).then(res => {
            // this.onload();
            this.$emit('refreshBtn');
            this.activeBtn = {};
            this.handleClose();
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            this.multipleLeftButtonDefinitionEdit(this.ruleForm).then(res => {
              this.$emit('refreshBtn');
              this.activeBtn = {};
              this.handleClose();
            });
