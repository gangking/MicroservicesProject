...
export default {
  ...
  methods: {
    ...
    deleteDef() {
      const newData = []
      const data = this.bpmAuth.rightsDef
      const selected = this.defineSelect
      let isHas = false
      for (let i = 0; i < data.length; i++) {
        isHas = false
        for (let j = 0; j < selected.length; j++) {
          if (selected[j].name === data[i].name) {
            isHas = true
          }
        }
        if (!isHas) {
      ...
      this.bpmAuth.rightsDef = newData
    },
    setBpmAuth() {
    ...
    rowClick(selection, row) {
      ...
      for (let i = 0; i < selection.length; i++) {
        this.defineSelect.push(selection[i])
      }
    },
    meditChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['m_edit'] = value
      })
    },
    mdelChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['m_del'] = value
      })
    },
    mstartChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['m_start'] = value
      })
    },
    msetChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['m_set'] = value
      })
    },
    mcleanChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['m_clean'] = value
      })
    },
    mguideEditChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['m_guideEdit'] = value
      })
    },
    idelChange(value) {
      this.bpmAuth.rightsDef.forEach(element => {
        element.rights['i_del'] = value
      })
    }
  }
