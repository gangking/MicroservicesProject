  ...
  <div class="tabelbox">
    <div class="list">
      ...
      <el-table height="100%" ref="multipleTable" :data="tableData" tooltip-effect="dark" header-align="center" style="width: 100%">
        ...
        <el-table-column prop="oneAuditPoolName" label="前置稽核池">
        </el-table-column>
        <el-table-column prop="twoAuditPoolName" label="稽核池">
        </el-table-column>
        <el-table-column prop="twoAuditPoolName" label="流程名称">
          <template slot-scope="scope">
            <p v-for="(item,index) in scope.row.bpmList" :key="index">{{item.bpmName}}</p>
          </template>
        </el-table-column>
        <el-table-column prop="twoAuditPoolName" label="稽核环节">
          <template slot-scope="scope">
            <p v-for="(item,index) in scope.row.bpmList" :key="index">{{item.nodeName}}</p>
          </template>
        </el-table-column>
