<template>
  <div class="automaticbox">
    <div v-if="!isAddEdit">
      ...
      <tableComponent :pageResult="pageResult" @handleCurrentChange="handleCurrentChange" :activeRow="activeRow" :tableData="tableData" @getGroupGoPage="getGroupGoPage" @btnEditClick="btnEditClick" @btnStartClick="btnStartClick" @btnStopClick="btnStopClick" @btnDeleteClick="btnDeleteClick" @getMultipleSelection="getMultipleSelection" />
    </div>
    <div v-else>
    ...
    btnClick(type) {
      switch (type) {
        ...
        case '保存自动规则':
          ...
          break;
        case '显示已选规则':
          console.log('该业务组已选的规则', this.busSet.autoRuleId, this.tableData);
          for (var i = 0; i < this.tableData.length; i++) {
            var item = this.tableData[i];
            if (item.id == this.busSet.autoRuleId) {
        ...
        .then(() => {
          if (this.multipleList.length > 0) {
            this.deliverRuleSave({
              ...
              autoRuleId: this.multipleList[0].id,
              manRuleId: this.busSet.ruleId
            }).then(res => {
              this.closeBox();
            });
          } else {
