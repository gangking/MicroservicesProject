<template>
  <div class="bg" ref="main" v-loading="bgLoading" element-loading-text="发送中,请等待">
    <div class="redblock_title" ref="title">
      ...
      <label>单据满意度</label>
      <i class="el-icon-search search" style="margin-left:10px;" @click="handler('openSearch')"></i>
    ...
    <div class="table-content" :style="{height:height+'px'}">
      ...
      >
        ...
        <el-table-column label="单据编号" align="center" sortable width="150">
          <template slot-scope="scope">
            <el-link @click="routeForm(scope.row)">{{scope.row.docnumber}}</el-link>
            <!-- <span class="routeForm" @click="routeForm(scope.row['单据编号'])">{{scope.row['单据编号']}}</span> -->
          </template>
        </el-table-column>
        <el-table-column prop="bsDesc" label="业务场景" showOverflowTooltip align="center" sortable></el-table-column>
        <el-table-column
          prop="docDescription"
          label="单据描述"
          showOverflowTooltip
          ...
          sortable
        ></el-table-column>
        <el-table-column label="完成时间" align="center" sortable width="140">
