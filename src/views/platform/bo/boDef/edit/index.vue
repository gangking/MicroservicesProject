...
export default {
  ...
  methods: {
    ...
    nodeStatus(value, use) {
      ...
      if (use === 'rights') {
        console.log(target, show, 'nodeStatus')
        return show
      } else {
        return isMaster
      }
      ...
      const sanmeNode = (data, param) => {
        data.find(item => {
          if (param === 'boDef') {
            ...
            if (item.code === value.code && (value.edit && value.status === '')) {
              isSame = true
    ...
    changeData(value, tabName) {
      ...
      if (value.code === '' && value) {
        return
      } else if (this.tabName === 'basic-info' && this.checkSameNode(value, 'new') && this.action === 'addSub') {
        this.$message({
