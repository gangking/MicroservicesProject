...
export default {
  ...
  actions: {
     ...
     * @param {Object} state vuex state
     * @param {Object} param { index, params, query, name } 路由信息
     */
     ...
     * @param {Object} state vuex state
     * @param {Object} param { name, params, query, newName } 路由信息
     */
    open({ state, commit, dispatch }, { name, params, query, newName }) {
      return new Promise(async resolve => {
        if (!state.init) {
    ...
    closeAll({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        ...
        if (router.app.$route.name !== INDEX_TAG) {
          router.push({
            name: INDEX_TAG
          })
        }
  ...
  mutations: {
    ...
    * @param {Object} state vuex state
    * @param {String} id 删除的任务台id
    */
    menuSingleDele(state, id) {
      state.menuDele = id
    },
    /**
  
   * @param {Object} state vuex state
   * @param {String} tabs 删除的tabs包含的所有id
   */
    menuTabDele(state, tabIds) {
      state.menuTabs = tabIds
    ...
    * @param {Object} state vuex state
    * @param {String} mentData 删除的tabs包含的所有id
    */
    menuAllDataDele(state, allData) {
      console.log(allData);
      state.menuAllData = allData
