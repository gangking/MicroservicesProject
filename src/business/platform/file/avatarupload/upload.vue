    ...
    <div class="header">
      <div class="btns">
        <el-upload
          ...
          action="https://www.pactera.com/post"
          list-type="picture"
          name="file"
        >
          <el-button slot="trigger" icon="el-icon-upload" type="primary">上传头像</el-button>
          <!-- <el-button type="danger" icon="ibps-icon-remove" @click="clearFiles">全部删除</el-button> -->
        </el-upload>
      </div>
    </div>
    <div class="uploader">
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (this.fileSize && file.size > this.fileSize * 1024) {
        ...
        this.$message({
          message:
            "上传文件的尺寸大于" + this.$utils.formatSize(this.fileSize * 1024),
          type: "warning"
        });
        return false;
      }
      // accept中的中生成匹配正则。
      // console.log(this.accept, 'accept中的中生成匹配正则1。')
      console.log(
        this.accept && this.handleAccpt(file),
        "accept中的中生成匹配正则2。"
      );
      console.log(this.fileExtType(file), "accept中的中生成匹配正则3。");
    ...
    handleSuccess(response, file, fileList) {
      ...
      this.$emit("callback", this.filePath)
    ...
    handleError(err, file, fileList) {
      ...
      if (!(err instanceof Error)) {
        ...
        this.$message.closeAll();
        this.$message({
          message: this.$utils.isNotEmpty(data.message)
            ? data.message
            : data.cause,
          type: "error"
        });
    ...
    emitCallback(list) {
      // console.log(this.getFileDataList(list));
      // let rtn;
      // if (this.multiple) {
      //   rtn = this.$utils.isNotEmpty(list) ? this.getFileDataList(list) : [];
      // } else {
      //   rtn = this.$utils.isNotEmpty(list)
      //     ? list[list.length - 1].response.data
      //     : "";
      // }
      // console.log(rtn);
     ...
     */
    clearFiles() {
      const ids = this.$refs.upload.uploadFiles
        .map(file => {
