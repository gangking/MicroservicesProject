<template>
  <el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" :title="title" class="bpmn-reject-dialog" append-to-body top="5vh" width="60%" @open="getFormData" @close="closeDialog">
    <el-form ref="form" v-loading="dialogLoading" :element-loading-text="$t('common.loading')" :model="form" :rules="rules" label-width="120px">
      ...
      <el-form-item v-if="actionName === 'reject' || actionName === 'rejectToStart' " label="返回方式:" prop="backHandMode">
      ...
      <el-form-item v-if="actionName === 'reject' && form.rejectMode==='rejectDest'" prop="destination">
        ...
        <el-select v-model="form.destination" placeholder="请选择" style="margin-top: 20px;">
          <el-option v-for="item in userNodeList.filter((_val) => {

              return actionName == 'reject' && _val.nodeId != 'UserStart'
            }) " :key="item.nodeId" :value="item.nodeId" :label="item.name" />
        </el-select>
      </el-form-item>
      <el-form-item label="审批意见:" prop="opinion">
        <approval-opinion v-model="form.opinion" />
      </el-form-item>

    </el-form>
    <div slot="footer" class="el-dialog--center">
      <el-button type="primary" icon="ibps-icon-save" @click="handleSave()">回退</el-button>
  ...
  computed: {
    title() {
      if (this.actionName === 'rejectToPrevious') {
        ...
        if (this.actionName2 === 'returnOfPreExam') {
          return '初审退回';
        } else if (this.actionName2 === 'documentBackflush') {
          return '单据反冲';
        } else {
          return '驳回发起人';
        }
  ...
  methods: {
    changeBackHandMode() {
      if (this.actionName === 'reject' && this.form.rejectMode === 'rejectDest') {
        this.formValidate();
        this.userNodeList =
          this.form.backHandMode === 'normal' ? this.bpmExecGoMapUserNode : this.bpmExecUserNode;
      }
    },
    closeDialog() {
        ...
        case 'reject': // 驳回
          toReject({
            ...
            .then(response => {
              ...
              // console.log('this.userNodeList', this.userNodeList)
