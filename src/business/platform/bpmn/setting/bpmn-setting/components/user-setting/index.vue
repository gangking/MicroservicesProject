<template>
  <div class="form-rule-part" ref="main">
    <fieldset class="scheduler-border">
      <legend style=" width: 90px;border-bottom:0;margin-bottom:10px;">
        <span :style="'marginLeft:8px'">人员设置</span>
      </legend>
      ...
      <div class="user-form" :style="{width:userWith}">
        <el-table
          ref="userTable"
          :data="userList"
          border
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      this.userWith = this.$refs.main.clientWidth + "px";
    });
  },
  computed: {
    ...mapState({
    ...
    userTypeTitle() {
      return this.userType && this.pluginTypeMap
        ? this.pluginTypeMap[this.userType].label
        : "";
    },
    pluginTypeMap() {
      const pluginTypeMap = {};
  ...
  methods: {
    ...
    changeUserType(type, index) {
      ...
      this.userList[index].description =
        this.pluginTypeMap[type].descText || "";
      const propertys = [
        "pluginType",
        "source",
        "description",
