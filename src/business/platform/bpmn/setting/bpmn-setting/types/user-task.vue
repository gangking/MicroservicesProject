<template>
  <div>
    <div class="setting-title" @click="onTitle">用户任务设置--【{{ data.node_name }}】</div>
    <el-collapse v-model="activeNames">
      <el-collapse-item title="用户设置" name="userSetting">
        <user-setting :data="users" />
      </el-collapse-item>
      <!-- 隐藏掉 -->
      <el-collapse-item title="表单设置" name="fromSetting" v-if="false">
        <form-setting
          :data="formData"
          :bo-data="boData"
          :def-key="defKey"
      ...
      <!-- 业务场景功能配置文件关联 -->
      <!-- <el-collapse-item title="业务场景功能配置文件关联" name="configChain">
        <config-chain :nodeId="data.id" :userTaskData="userTaskData" @splicingForm="splicingForm" />
      </el-collapse-item> -->
      ...
      <!-- 隐藏掉 -->
      <el-collapse-item title="按钮设置" name="buttonSetting" v-if="false">
...
// 业务场景配置关联
import ConfigChain from "../settings/config-chain";
export default {
  mixins: [mixins],
  components: {
    ConfigChain
  ...
  methods: {
    splicingForm(id,formData){
      this.$emit('splicingForm',id,formData)
    }
  },
  watch: {
    data: {
      handler(val) {
        // console.log("当前nodeid", val.id);
      },
      immediate: true
    }
  },
