  ...
  <div v-loading="loading"
    ...
    <el-timeline v-if="opinions && opinions.length >0">
      <template v-if="hasApproval">
        <el-timeline-item v-for="(item, index) in opinions"
                          :key="index"
                          :type="item.status==='pending'?'danger':'success'"
                          :color="item.status | filterColor"
                          :hide-timestamp="true"
                          placement="top"
                          size="large">
          ...
          <el-card>
            <div slot="header"
              ...
              <div class="ibps-fr">审批状态:<el-tag style="border: 0;"
                        :type="item.status==='pending'?'danger':'success'"> {{ item.statusName }}</el-tag>
              </div>
            </div>
            <p />
            <div v-if="item.status !== 'pending'"
                 class="ibps-clearfix">
              审批意见: {{ item.opinion||'无' }}
              <el-link v-if="item.batch"
                       class="ibps-fr"
                       type="primary"
                       @click="handleMore(item.batch,item.taskName)">更多</el-link>
            </div>
            <p />
            <div class="footer ibps-clearfix">
              ...
              <div v-if="$utils.isNotEmpty(item.completeTime)"
                   class="ibps-fr">
        ...
        <el-timeline-item v-for="(item, index) in opinions"
          ...
          <el-card>
            <div class="footer ibps-clearfix">
              ...
              <div>
                <span v-if="item.name">
                  <el-tooltip content="暂定的有审批资格的用户及用户组，实际候选人要在产生任务以后才能确定"
                              placement="bottom">
                    <abbr> 暂定候选人</abbr>
              ...
              <p />
              <div>审批状态:<el-tag style="border: 0;"
                        type="info">未执行任务</el-tag>
              </div>
    ...
    <el-alert v-else
              ...
              style="height:50px;" />
    <!--流程历史-->
    <approval-history-sign :visible="approvalHistorySignVisible"
                           :title="signTitle"
                           :batch="signBatch"
                           @close="visible => approvalHistorySignVisible= visible" />
    <!--流程历史-->
    <approval-history-call-sub :visible="approvalHistoryCallSubVisible"
                               :title="callSubTitle"
