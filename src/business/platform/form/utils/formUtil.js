...
import { dataFormatMap } from '../formbuilder/constants'
...
const FormUtil = {
   ...
   */
    ...
    getCurrentDataValue({ selectorType, bindId, name, storeType }, callback) {
        ...
        const paramsVal = this.getSelectorValue(storeType, this.CACHE_CURRENT_USER_DATA[selectorType])
        // 回调数据
        callback(name, paramsVal)
        return paramsVal
   ...
   */
    ...
    getAddressValue(value, model, fieldOptions) {
        ...
        for (let i = topIndex, j = 0; i < levelIndex + 1; i++, j++) {
