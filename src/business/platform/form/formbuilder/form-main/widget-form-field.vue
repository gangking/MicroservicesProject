<template>
  <div>
    ...
    <template v-else-if="field.field_type === 'radio'">
      <el-radio-group :value="defaultValue" disabled>
        <component
    ...
    <template v-else-if="field.field_type === 'checkbox'">
      <el-checkbox-group :value="defaultValue" disabled>
        <component
    ...
    <template v-else-if="field.field_type == 'switch'">
      <el-checkbox v-if="fieldOptions.appearance==='checkbox'" :value="defaultValue" disabled />
      <el-switch v-else :value="defaultValue" disabled />
    </template>

    <!-- 滑块 -->
    <template v-else-if="field.field_type === 'slider'">
      <el-slider :value="defaultValue" disabled />
    </template>
    <!-- 隐藏域 -->
    <template v-else-if="field.field_type === 'hidden'">
      <el-input :value="defaultValue" disabled>
        <i slot="prefix" class="ibps-icon-eye-slash red" />
      </el-input>
    </template>
    <!-- 远程搜索 -->
    <div v-else-if="fieldType==='remoteSearch'">
      <fssc-remote-search
        :value="defaultValue"
    ...
    <el-input
      ...
      disabled
    >
      <el-button slot="append" :icon="icon" />
    </el-input>
    <!-- fssc自定义对话框-->
    <el-input
      v-else-if="fieldType==='fsscCustom'"
      :value="defaultValue"
      :type="fieldOptions.type"
      :placeholder="fieldOptions.placeholder"
      disabled
    ...
    <!-- 签名-->
    <div v-else-if="fieldType==='signature'" class="fr-signature-outer">
    ...
    <!-- =======================流程字段==============================-->
    <!-- 流程图-->
    <img v-else-if="fieldType==='flow_diagram'" :src="flowDiagramImage" />
    <!-- 审批历史-->
    <img
    ...
    <!-- =======================其它字段==============================-->
    <!-- 文本-->
    <span v-else-if="fieldType==='label'">{{ field.value|| '用于展示文本或者公式计算的值,且数据不会保存' }}</span>
    <ibps-link
    ...
    <div v-if="element && element.desc" class="ibps-help-block">{{ element.desc }}</div>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    fieldOptions() {
     // console.log(this.field.field_options, "this.field.field_options");
    ...
    autosize() {
      ...
      if (
        this.field.field_options.autosize &&
        (this.field.field_options.min_rows || this.field.field_options.max_rows)
      ) {
        const row = {};
        row.minRows = this.field.field_options.min_rows
          ? this.field.field_options.min_rows
          : null;
        row.maxRows = this.field.field_options.max_rows
          ? this.field.field_options.max_rows
          : null;
        autosize = row;
      }
      return autosize;
    ...
    topVal() {
      ...
      if (this.field.field_options.topval) {
        return this.field.field_options.topval[
          this.field.field_options.topval.length - 1
        ];
      } else {
        return void 0;
      }
