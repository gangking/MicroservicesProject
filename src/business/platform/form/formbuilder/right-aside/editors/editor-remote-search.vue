  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      ...
      <el-form-item>
        <template slot="label">
          主动更新
          <help-tip prop="isUpdate" />
        </template>

        <el-switch v-model="fieldOptions.isUpdate"></el-switch>
      </el-form-item>
      <el-form-item>
        <template slot="label">
          查询字段
...
import remoteLinkDialog from "../components/remote-link-dialog"; // 联动数据弹窗
import remoteConditionDialog from "../components/remote-condition-dialog"; // 联动数据弹窗
...
export default {
  ...
  methods: {
    ...
    loadRemoteSource() {
      getRemoteSearch({
        parameters: [{ key: "Q^TYPE_^S", value: "valueSource" }],
    ...
    getDataChain() {
      queryRemoteData({ dataTemplateId: this.fieldOptions.search_url }).then(
        response => {
          // console.log(response, "response查询的数据");
          this.dataSource = [];
          this.dataCondition = [];
          if (response.data[0].resultColumns) {
            let resultColumns = JSON.parse(response.data[0].resultColumns);
            let copyColumns = JSON.parse(response.data[0].resultColumns);
          ...
          if (response.data[0].queryColumns) {
            ...
            if (resultCondition.length > 0) {
              for (let list1 of resultCondition) {
                for (let item1 of this.fieldItem.field_options.data_condition) {
