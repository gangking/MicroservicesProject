<template>
  <div class="formbuilder-container">
    <ibps-layout ref="layout">
      <div slot="west">
        <left-aside :bo-data="boData" />
      </div>
      <div class="form-main">
        <el-header :height="'40px'" class="layout-header form-actions">
          <div class="layout-header-title">表单设计</div>
...
<script>
import { save } from "@/api/platform/form/formDef"; // 保存表单信息api
import ActionUtils from "@/utils/action"; // 工具集合 包含提示信息
// import { testKey } from '@/utils/validate'
import FormOptions from "@/business/platform/form/constants/formoptions"; // 表单选项，是否可以支持
import LeftAside from "./left-aside"; // 表单左侧
import RightAside from "./right-aside"; // 表单右侧
import WidgetForm from "./form-main/widget-form"; // 整个表单中间
import FormrenderPreview from "@/business/platform/form/formrender/preview/index.vue"; // 表单预览
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (field.field_type === "grid") {
          ...
          if (this.$utils.isNotEmpty(columns)) {
            for (let j = 0; j < columns.length; j++) {
              const column = columns[j];
              const msg = this.checkField(column);
              if (msg) {
                this.selectField(column, code);
                callback(false, msg);
                return;
              }
            }
