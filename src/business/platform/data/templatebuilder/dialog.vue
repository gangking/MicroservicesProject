<template>
  <el-dialog
    ...
    @close="closeDialog"
  >
    <div v-loading.fullscreen.lock="dialogLoading" :element-loading-text="$t('common.loading')">
...
export default {
  ...
  methods: {
    ...
    ...mapMutations("ibps/form", ["formTemplate"]),
    // 关闭当前窗口
    closeDialog() {
    ...
    getDefaultTemplateAttrs(data) {
      if (data.showType === "list") {
        data.templates = [Utils.defaultListAttrs()];
      }
    ...
    getFormData() {
      ...
      if (
        this.$utils.isEmpty(this.formId) &&
        this.$utils.isNotEmpty(this.data)
      ) {
        const data = JSON.parse(JSON.stringify(this.data));
        // 重置表单
        this.dataTemplate = _defaultsDeep(
          this.getDefaultTemplateAttrs(data),
          defaultForm
        );
