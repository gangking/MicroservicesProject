    ...
    <div class="panel-body">
      ...
      >
        <div v-for="(button,i) in buttons" :key="button[buttonKey]+i" class="list-group-item">
          <div class="actions-left">{{ button.label }}</div>
...
export default {
  ...
  computed: {
    ...
    functionButtons() {
      const buttons = [];
      console.log(this.types, "9999999999");
  ...
  methods: {
    ...
    settingButton(i) {
      this.editData = _defaultsDeep(
        JSON.parse(JSON.stringify(this.buttons[i])),
        this.defaultValue
      );
      this.editIndex = i;
    ...
    handleConfirm() {
      this.buttons.splice(this.editIndex, 1, this.editData);
      this.handleInput(this.buttons);
      this.closeDialog();
    },
    handleInput(value) {
