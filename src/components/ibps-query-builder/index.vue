...
export default {
  ...
  props: {
    ...
    labels: {
      ...
      default: () => {
        return {
          'addRulue': I18n.t('components.queryBuilder.add_rule'),
          'addGroup': I18n.t('components.queryBuilder.add_group'),
          'removeRule': I18n.t('components.queryBuilder.remove_rule'),
          'removeGroup': I18n.t('components.queryBuilder.remove_group')
        }
      }
    },
    conditions: {
      type: [Object, Array],
      default: () => {
        return [{ value: 'AND', label: I18n.t('components.queryBuilder.conditions.AND')
        }, {
          value: 'OR', label: I18n.t('components.queryBuilder.conditions.OR')
        }]
      }
    },
    sort: Boolean, // 排序
  ...
  watch: {
    rules: {
      handler(val, oldVal) {
        this.$emit('input', Utils.deepClone(val))
      },
      deep: true
    }
  },
  mounted() {
    if (typeof this.$options.propsData.value !== 'undefined') {
      this.rules = Object.assign(this.rules, this.$options.propsData.value)
    }
  },
  methods: {
    handleMergedFilters(filters) {
      ...
      filters.forEach((filter) => {
        ...
        filter.operators = this.getOperators(filter)
    ...
    getOperators(filter) {
      const result = []
      Object.keys(QueryBuilder.OPERATORS).forEach((op) => {
        const operators = QueryBuilder.OPERATORS[op]
        // filter operators check
        if (filter.operators) {
          if (filter.operators.indexOf(operators.type) === -1) {
            return true
          }
        // type check
        } else if (operators.apply_to.indexOf(QueryBuilder.TYPES[filter.type]) === -1) {
          return true
        }
