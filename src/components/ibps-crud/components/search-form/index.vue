...
export default {
  ...
  inject: ["formData", "searchFile"],
  watch: {
    getFormData: {
      handler(val, oldVal) {
        //   console.log(val, "来自provide的数据");
        this.handlerForm();
      },
      immediate: true,
      deep: true
  ...
  methods: {
    handlerForm() {
     // console.log(this.getSearchFile, "字段关联88888");

      for (const key in this.getFormData) {
        let keySL = "Q^" + key + "^SL";
        const element = this.getFormData[key];

        this.getSearchFile.forEach(item => {
          let itemId = "Q^" + item.select + "^SL";
          let itemName = "Q^" + item.name + "^SL";
          if (keySL == itemId) {
            this.params[itemName] = element;
          } else if (item["fix_value"]) {
            this.params[itemName] = item["fix_value"];
          }
        });
      }
    },
